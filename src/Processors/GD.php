...
class GD extends Processor {
    ...
    private function _createImage(?string $path = null) {
        $path = is_null($path) && !is_null($this->config->getFullPath()) ? $this->config->getFullPath() : trim($path);
    ...
    protected function _retrieveImageSize() {
        $imageInfo = @getimagesize($this->config->getFullPath());
        if ($imageInfo === false) {
            throw new \RuntimeException("Failed to retrieve image size for file: " . $this->config->getFullPath());
        }
    ...
    protected function _saveImage() {
        $ext = pathinfo($this->config->getFullPath(), PATHINFO_EXTENSION);
        switch(strtolower($ext)) {
            case "png":    imagepng($this->image['obj'], $this->config->getFullPath()); break;
            case "jpg":   imagejpeg($this->image['obj'], $this->config->getFullPath()); break;
            case "jpeg":  imagejpeg($this->image['obj'], $this->config->getFullPath()); break;
            case "gif":    imagegif($this->image['obj'], $this->config->getFullPath()); break;
            case "webp":  imagewebp($this->image['obj'], $this->config->getFullPath()); break;
    ...
    public function suggestTilesQuantity() {
        ...
        if(is_null($this->image['obj']))
            $this->_createImage($this->config->getFullPath());
    ...
    public function output() {
        if($this->config->getFullPath() == null) return false;
        $this->_createImage();
